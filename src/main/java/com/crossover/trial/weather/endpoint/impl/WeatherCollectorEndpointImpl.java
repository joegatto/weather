...
public class WeatherCollectorEndpointImpl implements WeatherCollectorEndpoint {
    ...
    @Override
    public Response updateWeather(@PathParam("iata") final String iataCode, @PathParam("pointType") final String pointType,
    ...
    public Response addAirport(@PathParam("iata") final String iata, @PathParam("lat") final String latString,
        ...
        try {
            airport = new Airport.Builder().withIata(iata).withLatitude(Double.valueOf(latString)).withLongitude(Double.valueOf(longString)).build();
        } catch (IllegalArgumentException e) {
            LOGGER.log(Level.SEVERE, "Wrong airport coordinates latString = " + latString + ", longString = " + longString);
            return Response.status(Response.Status.BAD_REQUEST).build();
        }
