...
public class WeatherQueryEndpointImpl implements WeatherQueryEndpoint {
	...
	public String ping() {
		...
		// fraction of queries
		for (Airport data : airportService.getAirports()) {
			double frac = (double) airportService.getRadiusCounts().getOrDefault(data, 0)
		...
		int[] hist = new int[m];
		for (Map.Entry<Double, Integer> e : airportService.getRadiusCounts().entrySet()) {
	...
	private Airport findAirport(final String iataCode) {
		return airportService.getAirports().stream().filter(ap -> ap.getIata().equals(iataCode)).findFirst()
				.orElse(null);
	}
